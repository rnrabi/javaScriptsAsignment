...
// This function will identify the negetive marks from a array.

function findingBadData(bdData){
    if(Array.isArray(bdData) === true){
        let badData = 0;
        for(let i = 0; i < bdData.length; i++){
            let element = bdData[i];
            if(element < 0){
                badData = badData + 1;
            }
            else{
...
let result = findingBadData([ -4,-2,-5,-7,-13 ]);
console.log(result);
